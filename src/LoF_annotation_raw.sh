...
# if ANALYSIS_TYPE empity 
    ...
    if [[ ${ANALYSIS_TYPE} == "ALL" ]] || [[ ${ANALYSIS_TYPE} == "VEPLOFTEE" ]]; then 
	echo ""
	importantnote "### VEP + LOFTEE"
...
### STEP 2 ###
    if [[ ${ANALYSIS_TYPE} == "ALL" ]] || [[ ${ANALYSIS_TYPE} == "LOFCALC" ]]; then
        echo ""
        importantnote "### Calculation of LoF genes and variants"
...
#	    echo "mv ${ROOTDIR}/${PROJECTNAME}_Files_for_VCF_LoF/vcf_chr*/*vep.vcf.gz ${OUTPUTDIR}" >> ${OUTPUTDIR}/run_vep_to_lof_gene.sh
		...
		sbatch --job-name=LoF_gene_${PROJECTNAME} -e LoF_gene_${PROJECTNAME}.error -o LoF_gene_${PROJECTNAME}.log -t ${QUEUE_LOF_GENE} --mem=${VMEM_LOF_GENE} --mail-user=${EMAIL} --mail-type=${MAILTYPE} -D ${OUTPUTDIR} ${OUTPUTDIR}/run_vep_to_lof_gene.sh
	    else
...
#============================================================================#
    if [[ ${ANALYSIS_TYPE} == "ALL" ]] || [[ ${ANALYSIS_TYPE} == "STAT" ]]; then
        echo ""
        importantnote "### Descriptive Statistics "
