...
while ( <$in> ) {
  chomp;
  my @fields = split /\t/;
  my ( $gene_ID , $gene_symbol , $single_copy_LoF_frequency, $two_copy_LoF_frequency ) = ( shift @fields , shift @fields, shift @fields , shift @fields ); #Explicitly assign these first fields, which don't have LoF strings.
  print {$out} join "\t" , ( $gene_ID , $gene_symbol , $single_copy_LoF_frequency , $two_copy_LoF_frequency );
  for ( @fields ) {
